...
class _AskedQuestionsState extends State<AskedQuestionsScreen> {
  ...
  Container buildProgressCaption(BuildContext context) {
    return Container(
  ...
  Container buildMenuButton(BuildContext context) {
    return Container(
      ...
      }, 'sans-serif', 'menu'.tr(), MediaQuery.of(context).size.width * 0.06,
          null, null),
    );
  }
  ...
  Container buildStatistics(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: BorderRadius.all(Radius.circular(4))),
...
class DiagramWidget extends StatefulWidget {
  ...
  DiagramWidget({Key key, this.chartKey}) : super(key: key);
...
class _DiagramWidgetState extends State<DiagramWidget> {
  ...
  void initState() {
    super.initState();
    AwarenessProgress awarenessProgress =
        myDbBox.get(MyResource.AWARENESS_PROGRESS_KEY);
    _progress = (awarenessProgress != null) ? awarenessProgress.awareness : 0;
  }
